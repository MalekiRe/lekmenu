use std::sync::Arc;
use anyhow::Result;
use stardust_xr_fusion::client::{Client, LifeCycleHandler, LogicStepInfo};
use stardust_xr_fusion::drawable::{Text, TextStyle};
use stardust_xr_fusion::{client, resource};
...
async fn main() -> Result<()> {
    ...
    let field = stardust_xr_fusion::fields::SphereField::builder()
			...
			.build()
			.unwrap();
    let pulse_reciver = PulseReceiver::create(client.get_root(), None, None, &field, KEYBOARD_MASK.clone().take_buffer(), |_, _| PulseReceiverTest(client.clone())).unwrap();
    tokio::select! {
